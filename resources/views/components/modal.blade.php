...
<div x-data="{ show: @entangle($attributes->wire('model')) }"
    ...
    x-on:keydown.escape.window="show = false; $wire.call('resetVariables')" x-show="show" id="{{ $id }}"
    class="jetstream-modal fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" style="display: none;">
    <div x-show="show" class="fixed inset-0 transform transition-all"
        x-on:click="show = false; $wire.call('resetVariables')" x-transition:enter="ease-out duration-300"
    ...
    <div x-show="show"
        class="mb-6 bg-white dark:bg-gray-800 rounded-lg dark:border dark:border-slate-700 overflow-hidden shadow-xl transform transition-all sm:w-full {{ $maxWidth }} sm:mx-auto"
