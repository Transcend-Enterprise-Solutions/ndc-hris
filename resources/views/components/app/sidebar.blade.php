    ...
    <div id="sidebar"
        ...
        <div class="flex justify-left mb-10 ml-1" style="width: 200px; height: 32px;">
            ...
            <a class="flex items-center" href="{{ route('dashboard') }}">  
                ...
                <span class="text-black dark:text-white ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">NYC - HRIS</span>          
            </a>
        </div>
...
<!-- Admin Tabs ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------->
                ...
                @if(Auth::user()->user_role != 'emp')
                    ...
                    @if(Auth::user()->user_role === 'sa' || Auth::user()->user_role === 'hr')
                        ...
                        <!-- Role Management -->
                        <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-[linear-gradient(135deg,var(--tw-gradient-stops))] 
                            @if(in_array(Request::segment(1), ['role-management'])){{ 'bg-gray-200 dark:bg-slate-900' }}@endif"                            
                            x-data="{ open: {{ in_array(Request::segment(1), ['role-management']) ? 1 : 0 }} }">
                            <a class="block text-gray-800 dark:text-gray-100 truncate transition 
                                @if(Route::is('role-management')){{ '!text-blue-500' }}@endif"
