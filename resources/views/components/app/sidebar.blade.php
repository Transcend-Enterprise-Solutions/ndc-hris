<div class="min-w-fit">
    ...
    <div id="sidebar"
    class="flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll lg:overflow-y-auto no-scrollbar shrink-0 bg-white dark:bg-slate-800 p-4 transition-all duration-200 ease-in-out"
    :class="{
    ...
    <div class="flex justify-left mb-10 ml-1" style="width: 200px; height: 32px;">
        ...
        <a class="flex items-center" href="{{ route('dashboard') }}">
            ...
            <span
                class="text-black dark:text-white ml-2 transition-opacity duration-200 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100"
                x-show="sidebarExpanded">NYC - HRIS</span>
        </a>
    </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('sidebar', () => ({
            ...
            expandSidebar() {
                ...
                this.$dispatch('sidebar-expanded', this.sidebarExpanded);
            }
        }));
    });
    
    window.addEventListener('sidebar-expanded', event => {
        document.body.classList.toggle('sidebar-expanded', event.detail);
    });
</script>
