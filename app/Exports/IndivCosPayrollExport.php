...
class IndivCosPayrollExport implements WithEvents, WithDrawings
{
    ...
    private function Footer($sheet, $payroll){
        ...
        $this->currentRow += 3;
        $sheet->mergeCells("A{$this->currentRow}:C{$this->currentRow}");
        $sheet->setCellValue("A{$this->currentRow}", $this->filters['preparedBy'] ? $this->filters['preparedBy']->name : 'XXXXXXXXXX');
        $sheet->mergeCells("D{$this->currentRow}:F{$this->currentRow}");
        $sheet->setCellValue("D{$this->currentRow}", $this->filters['notedBy'] ? $this->filters['notedBy']->name : 'XXXXXXXXXX');
        $sheet->getStyle("A{$this->currentRow}:F{$this->currentRow}")->getFont()->setBold(true);

        $this->currentRow++;
        $sheet->mergeCells("A{$this->currentRow}:C{$this->currentRow}");
        $sheet->setCellValue("A{$this->currentRow}", $this->filters['preparedBy'] ? $this->filters['preparedBy']->position : 'Position');
        $sheet->mergeCells("D{$this->currentRow}:F{$this->currentRow}");
        $sheet->setCellValue("D{$this->currentRow}", $this->filters['notedBy'] ? $this->filters['notedBy']->position : 'Position');
